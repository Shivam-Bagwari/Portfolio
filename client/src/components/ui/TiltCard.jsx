...
useEffect(() => {
  ...
  frame.current = requestAnimationFrame(() => {
    ...
    const ry = (px - 0.5) * 8;
     ...
     el.style.transform =
      `perspective(1400px) rotateX(${rx}deg) rotateY(${ry}deg) translateY(-4px) scale(1.01)`;
  ...
  return (
    ...
    >
      ...
      group-hover:opacity-100"
        ...
        style={{ background: "radial-gradient( 260px circle at var(--mx,50%) var(--my,50%),rgba(217,70,239,.20),transparent 45% )" }}
      />
      <div className="pointer-events-none absolute inset-0 rounded-[32px] opacity-0 transition-opacity duration-300 group-hover:opacity-100"
        style={{ background: "linear-gradient( 135deg, rgba(217,70,239,.70), transparent 42%, rgba(168,85,247,.45) )", WebkitMask: "linear-gradient(#000,#000) content-box, linear-gradient(#000,#000)", WebkitMaskComposite: "xor", maskComposite: "exclude", padding: "1px" }}
      />
      {children}
    </div>
  );
